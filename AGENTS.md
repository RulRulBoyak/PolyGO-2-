...
- `secrets.php` also holds (all gitignored): `GEMINI_API_KEY` (server-side AI proxy — never in the APK or `local.properties`), `ADMIN_PASSWORD` (shared admin-panel password for `admin/login.php`; the legacy `admin_verify.php` now redirects to the panel), and optional `APP_CHECK_ENFORCE` + `FIREBASE_PROJECT_NUMBER` (App Check). Out-of-band email (OTPs, temp passwords) goes through `Mailer.php` (PHPMailer via `vendor/`, composer-installed in `backend/polygo-api/`); with no SMTP configured it falls back to the PHP error log — never to the API response.
- Verification: `verify.php` handles upload/submit and a strict `status` read (banned accounts get 401 "suspended"); `admin/verification.php` (Unified Admin Control Center) approves/rejects. `config.php::verify_jwt()` auto-rejects posts from unverified accounts; the app mirrors this with `ui/VerificationGate.java`.
- Ban lifecycle: `admin/users.php` sets `users.is_banned`+`banned_at` (+ archives the user's listings); `login.php`/`google_login.php` return HTTP 401 "Unauthorized: Your account has been suspended by an administrator."; `config.php::verify_jwt()` → `isUserBanned()` blocks every JWT call (strict callers 401, optional callers get 0); `NotificationManager::sendToUser()` skips banned users (keeps their `fcm_token` so unban restores push without re-login); `admin/pulse.php` announcements exclude them. Android caches the flag (`AppDataStore.is_banned`), `NetworkModule`'s auth-response interceptor peeks 401 bodies for "suspended" → `AuthSessionHandler.onSuspended()` → `BannedActivity` (back-proof, clears the task), and `SplashActivity` cold-starts route there. Schema mirror: `migration_add_ban_fields.sql`.
- Admin live alerts: `admin/pulse.php` "announce" and `admin/admin_broadcast.php` "Remote Control" write one **Firestore `pulse` doc + FCM push per token + `campus_alerts` row**. The Firestore doc shape is a hard contract with `CampusPulseActivity`'s real-time `orderBy("created_at")` listener: it MUST contain `user_name` + integer-seconds `created_at` (`time()`), or the ordered query silently drops the doc (older docs with `author`/`createdAt` string fields were never delivered). `admin_broadcast.php` is the body-required "push to every device now" variant (title defaults "Live Announcement").
